...
class FlatRateEMICalculator extends StatefulWidget {
  @override
  _FlatRateEMICalculatorState createState() => _FlatRateEMICalculatorState();
}

class _FlatRateEMICalculatorState extends State<FlatRateEMICalculator> {
  ...
  void _calculateEMI() {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      double interest = double.parse(interestController.text) / 100;
      int period = int.parse(periodController.text);
      if (isYears) period *= 12;

      // Calculate total interest for the entire loan period
      totalInterest = amount * interest * (isYears ? int.parse(periodController.text) : period / 12);

      // Calculate monthly EMI
      monthlyEMI = (amount + totalInterest) / period;

      // Calculate processing fees
      processingFees = processingFeeController.text.isNotEmpty
          ? amount * double.parse(processingFeeController.text) / 100
          : 0;

      // Calculate total payment
      totalPayment = amount + totalInterest + processingFees;

      // Calculate flat rate (for comparison)
      flatRate = (totalInterest / amount / (period / 12)) * 100;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)?.flat_rate_emi_calculator ?? 'Flat Rate EMI Calculator'),
      ),
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                _buildInputField(
                    AppLocalizations.of(context)?.flat_interest_rate ?? 'Flat Interest Rate % (per annum)',
                    interestController,
                    AppLocalizations.of(context)?.enter_flat_interest ?? 'Enter flat interest rate'
                ),
                _buildPeriodField(),
                _buildInputField(
  ...
  Widget _buildResultsTable() {
    return Table(
      ...
      children: [
        ...
        _buildTableRow(AppLocalizations.of(context)?.flat_rate ?? 'Flat Rate', '${numberFormatter.format(flatRate)}%'),
      ],
    );
  }

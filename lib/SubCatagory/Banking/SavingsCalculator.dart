...
class _SavingsCalculatorState extends State<SavingsCalculator> {
  ...
  TextEditingController monthlySavingsController = TextEditingController();
  ...
  void _calculateSavings() {
    ...
    if (_formKey.currentState!.validate()) {
      double monthlySavings = double.parse(monthlySavingsController.text.replaceAll(',', ''));
      ...
      double futureValueOfSavings = monthlySavings * ((pow(1 + monthlyInterest, period) - 1) / monthlyInterest) * (1 + monthlyInterest);

      double totalAmountSaved = monthlySavings * period;
      double totalInterestEarned = futureValueOfSavings - totalAmountSaved;

      setState(() {
        futureValue = futureValueOfSavings;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(AppLocalizations.of(context)?.savings_calculator ?? 'Savings Calculator'),
      ),
        ...
        child: Padding(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: <Widget>[
                _buildInputField(AppLocalizations.of(context)?.monthly_savings ?? 'Monthly Savings', monthlySavingsController, AppLocalizations.of(context)?.enter_monthly_savings ?? 'Enter monthly savings amount'),
                ...
                Row(
                  children: [
                    Expanded(child: _buildButton(AppLocalizations.of(context)?.calculate ?? 'Calculate', _calculateSavings, Colors.blue)),
    ...
    return Column(
      ...
      children: [
        ...
        Table(
          ...
          children: [
            _buildTableRow(AppLocalizations.of(context)?.future_value ?? 'Future Value', currencyFormatter.format(futureValue)),
            _buildTableRow(AppLocalizations.of(context)?.total_savings ?? 'Total Savings', currencyFormatter.format(totalSavings)),
